...
test.describe('Product Browsing', () => {
  test('homepage shows hero section', async ({ page }) => {
    await page.goto('/')
    await expect(page.getByText('Auto Parts')).toBeVisible()
    await expect(page.getByText('in 30 Minutes')).toBeVisible()
  })
  ...
  test('product detail shows price and specs', async ({ page }) => {
    await page.goto('/products/1')
    await expect(page.getByText('$45.99')).toBeVisible({ timeout: 5000 })
