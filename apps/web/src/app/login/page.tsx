...
/**
 * Google login button — isolated in its own component so that
 * GoogleOAuthProvider only wraps this button, not the entire form.
 * If GOOGLE_CLIENT_ID is empty (CI, local dev), this renders a disabled placeholder.
 */
function GoogleLoginBtn({ onSuccess, onError, loading }: {
  ...
  if (!GOOGLE_CLIENT_ID) {
    return (
      <button type="button" disabled className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl bg-white text-gray-900 font-medium text-sm opacity-50 cursor-not-allowed">
  ...
  return <GoogleOAuthProviderInner onSuccess={onSuccess} onError={onError} loading={loading} />
}

function GoogleOAuthProviderInner({ onSuccess, onError, loading }: {
  onSuccess: (response: any) => void
  onError: () => void
  loading: boolean
}) {
